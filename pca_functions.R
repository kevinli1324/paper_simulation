...
run_pca_simulation <- function(file, gen_data, gen_labels, princomps,iter = 500, param_matrix = matrix(0, nrow = nrow(gen_data), ncol = 4), single = F, multicore = F, method = "princomp") {
  das_model <- stan_model("simple_multivar.stan")
  ...
  if(multicore){
    stan_models <- mclapply(data_list, run_stan, iter = iter)
    
  } else {
    stan_models <- lapply(data_list, run_stan, iter = iter, file = file)
  }
...
run_stan <- function(data, file, mean0 = 1, iter, chains) {
  if(nrow(data) == 0) {
    return(NA)
  ...
  fit <- stan(file = file, iter = iter, chains = chains, control = list(max_treedepth = 15), data = stanFeed)
  return(fit)
}
...
pca_wrapper <- function(data, princomps, multicore = F, method = "princomp", file, mean0 = rep(1, nrow(data)), clusters = 1, optimize = F, iter = 2000, chains = 1) {
  ...
  if (optimize == F) {
    if (multicore) {
      stan_models <-
        mclapply(data_list, run_stan,  mean0 = mean0, file = file)
      
    } else {
      stan_models <-
        lapply(data_list, run_stan, mean0 = mean0, file = file, iter = iter, chains = chains)
    }
  } else {
