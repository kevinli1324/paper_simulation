...
get_aij <- function(point, stan_frame , row) {
  mu0 <- fit_frame[[paste0("mu0[", row, "]")]]
  mu1 <- fit_frame[[paste0("mu1[", row, "]")]]
  mix <- fit_frame[[paste0("theta[",row,"]")]]
  sigma <- fit_frame[[paste0("sigma[",row,"]")]]
  mapply(calc_alpha, mu0 = mu0, mu1 = mu1, mix = mix, sigma = sigma, point = point)
}
aij_interval <- function(aijs, CI = .95) {
  alphas <- aijs
  CI <- 1-CI
...
paper_class <- function(logfit, tfit) {
  ifelse(abs(logfit) > .5 & abs(tfit) > 2, 1, 0)
}
